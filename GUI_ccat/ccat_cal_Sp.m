function FC_image = ccat_cal_Sp(InputPath, OutputPath, ROIPath, TR, Slices)
% %------------------------------------------------------------------------
...
InputParameter.RemoveFirstTimePoints.OutDirFunImg = [rawPath filesep 'FunImgT'];
...
FunFile=spm_select('FPList',FunName,'^RA.*\.nii');
for j=1:230
    jnum = num2str(j);
...
ref=spm_select('FPList', [rawPath filesep 'T1Img' filesep Funlist(3).name],'^T1.*\.nii');
...
% Smooth
mkdir ('FunImgARS')
InputParameter.Smooth.Gaussian.InDirFunImg = [rawPath filesep 'FunImgAR'];
InputParameter.Smooth.Gaussian.OutDirFunImg = [rawPath filesep 'FunImgARS'];
...
mkdir ('FunImgARSC')
mkdir ('CovParameter')
InputParameter.RegressOutCovariates.InDirFunImg = [rawPath filesep 'FunImgARS'];
InputParameter.RegressOutCovariates.OutDirFunImg = [rawPath filesep 'FunImgARSC'];
...
% Filter
mkdir ('FunImgARSCF')
InputParameter.Filter.InDirFunImg = [rawPath filesep 'FunImgARSC'];
InputParameter.Filter.OutDirFunImg = [rawPath filesep 'FunImgARSCF'];
...
if ROIPath == ccatPath
    ...
    OutDir = 'D:\GUI_CoorConv\OutResults';
    ...
    cd (ROIPath)
    ROI_defined = dir('*.nii');
    InputParameter.FunctionalConnectivity.InDirFunImg = [rawPath filesep 'FunImgARSCF'];
    InputParameter.FunctionalConnectivity.OutFodrFC = OutDir;
    ...
    for i = 1:rois
        ROIs = strcat(ROIPath, filesep, ROI_defined(i).name); % Select the ROI
        def = spm_select('FPList',[T1SegDir filesep sublist(3).name],'^iy_.*\.nii');
        matlabbatch{1}.spm.spatial.normalise.write.subj.def ={def} ;
